...
async def info(message: Message):
    ...
    kb = [
            ...
            [KeyboardButton(text="Метро и автобусы"), KeyboardButton(text="Культурные места")],
            [KeyboardButton(text="Интересные заведения")],
            [KeyboardButton(text="Опасные районы"), KeyboardButton(text="В меню")]
    ]
    keyboard = ReplyKeyboardMarkup(
        keyboard=kb,
        resize_keyboard=True,
...
async def info(message: Message):
    ...
    await message.answer("Check out each category, they will all help you get comfortable in a new place", reply_markup=keyboard)
...
@dp.message(F.text.lower() == "metro and buses")
async def metro(message: Message):
    await message.answer("'текст про то как оринтироваться в метро и тд'", reply_markup=ReplyKeyboardRemove())
    kb = [
        [
